...
public class PostService {
    ...
    public Page<Post> getPosts(UserEntity currentUser, Pageable pageable) {
        var postEntities = postEntityRepository.findAll(pageable);
        return postEntities.map(postEntity -> getPostWithLikingStatus(postEntity, currentUser));
    }
    ...
    public Page<Post> searchPostsByBody(String keyword, UserEntity currentUser, Pageable pageable) {
        if (keyword == null || keyword.trim().isEmpty()) {
            return Page.empty();
        }

        var postEntities = postEntityRepository.findByBodyContainingIgnoreCase(keyword, pageable);
        return postEntities.map(postEntity -> getPostWithLikingStatus(postEntity, currentUser));
    }
}
